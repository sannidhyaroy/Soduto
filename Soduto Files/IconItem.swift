...
public protocol IconItemDelegate: AnyObject {
...
public class IconItem: NSCollectionViewItem {
    ...
    public var fileItem: FileItem? {
        didSet {
            ...
            Logger.ui.debug("IconItem fileItem.didSet: \(self.fileItem?.name ?? "nil", privacy: .public)")
            ...
            if let fileItem = self.fileItem, !fileItem.flags.contains(.isDeleted) {
                ...
                guard fileItem.fileSize > 0 else {
                    Logger.ui.debug("IconItem: Skipping thumbnail for \(fileItem.name, privacy: .public) (size 0)")
                    return
                }
                
                // 4. file size < max
                guard fileItem.fileSize <= maxThumbnailFileSize else {
                    Logger.ui.debug("IconItem: Skipping thumbnail for \(fileItem.name, privacy: .public) (size \(fileItem.fileSize, privacy: .public) exceeds limit)")
                    return
                }
                
                Logger.ui.debug("IconItem: Requesting thumbnail for \(fileItem.name, privacy: .public)")
    ...
    private func loadThumbnail(for fileItem: FileItem) {
        ...
        Logger.ui.debug("IconItem loadThumbnail: Loading data for \(urlToLoad.lastPathComponent, privacy: .public)")
        ...
        imageLoader?.loadThumbnail(for: urlToLoad) { [weak self] (image, error) in
            ...
            guard let self = self else {
                ...
                Logger.ui.debug("""
                    IconItem loadThumbnail completion: self is nil.
                        Request was for \(urlToLoad.lastPathComponent, privacy: .public).
                    """)
                return
            }
            ...
            guard self.currentLoadingURL == urlToLoad else {
                ...
                Logger.ui.debug("""
                    IconItem loadThumbnail completion: Stale request. Call reused.
                        Loaded \(urlToLoad.lastPathComponent, privacy: .public) but current is
                            \(self.currentLoadingURL?.lastPathComponent ?? "nil", privacy: .public).
                    """)
                return
            }
            ...
            if let image = image {
                ...
                Logger.ui.debug("IconItem loadThumbnail success: Set thumbnail for \(urlToLoad.lastPathComponent, privacy: .public)")
                ...
                if let imageLoaderError = error as? ImageLoaderError {
                    ...
                    isRetryable = false
                    Logger.ui.debug("IconItem loadThumbnail: Received unknown error type. Not retrying.")
                }
                
                if isRetryable {
                    Logger.ui.debug("""
                        IconItem loadThumbnail FAILED (Retryable) for \(urlToLoad.lastPathComponent, privacy: .public). Scheduling retry.
                        Error: \(error.localizedDescription, privacy: .public)
                        """)
                    self.scheduleRetry(for: fileItem)
                } else {
                    Logger.ui.debug("""
                        IconItem loadThumbnail FAILED (Non-retryable) for \(urlToLoad.lastPathComponent, privacy: .public).
                        Error: \(error.localizedDescription, privacy: .public)
                        """)
                }
            } else {
                ...
                Logger.ui.debug(
                    "IconItem loadThumbnail cancelled for \(urlToLoad.lastPathComponent, privacy: .public)")
            }
        }
    }
    ...
    private func scheduleRetry(for fileItem: FileItem) {
        ...
        retryTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: false) { [weak self] _ in
            ...
            guard self.fileItem?.url == fileItem.url else {
                Logger.ui.debug("IconItem: Retry timer fired, but cell was reused. Cancelling retry.")
                return
            }
            
            Logger.ui.debug("IconItem: Retry timer fired. Retrying load for \(fileItem.name, privacy: .public).")
